...
class PVTimeSeriesDataset(Dataset):
    ...
    def __init__(self, csv_path: str, seq_len=256, power_col='Power', feature_cols=None,
        ...
        # Where mean/std are nan (all missing), set to 0/1 to avoid NaNs
        ...
        self.starts = []
        # If T < seq_len we still allow start 0 and we'll pad in __getitem__
    ...
    def __getitem__(self, idx):
        ...
        if end <= self.T:
            ...
            # padding to the right if not enough length
            avail = self.T - s
            ...
            # padded part remains zeros and mask zeros
        ...
        sample = {
            'x': seq,               # normalized features with NaNs filled (numpy float32)
            'mask': mask,           # observed mask (1 observed, 0 missing) (numpy float32)
            'raw_slice': raw_slice, # original raw filled (numpy)
            'power_idx': power_idx
        }
        return sample
...
class Trainer:
    ...
    def prepare_input(self, x_np, mask_np):
        ...
        inp = torch.cat([x, agg_mask, noise], dim=2).permute(0, 2, 1)
        agg_mask_t = agg_mask.permute(0, 2, 1)
    ...
    def train(self):
        ...
        if cfg.pretrain_epochs > 0:
            print(f"=== Pretraining generator with L1 for {cfg.pretrain_epochs} epochs ===")
        ...
        for epoch in range(cfg.epochs):
            ...
            for batch in pbar:
                ...
                for _ in range(cfg.n_critic):
                    ...
                    # 拼接 mask 和 noise，保证输入格式一致
                    fake_full = torch.cat([fake_det, agg_mask, self.sample_noise(B, L)], dim=1)
                    real_full = inp_real

                    real_score_g = self.Dglob(real_full)
                    fake_score_g = self.Dglob(fake_full)
                    real_score_l = self.Dlocal(real_full)
                    fake_score_l = self.Dlocal(fake_full)

                    d_loss = fake_score_g.mean() + fake_score_l.mean() - real_score_g.mean() - real_score_l.mean()

                    # 修复梯度惩罚：real 与 fake 必须通道数一致
                    gp = gradient_penalty(self.Dglob, real_full, fake_full, device=device)
                ...
                self.Dlocal.eval()
                fake = self.G(inp_real, agg_mask)
                fake_full = torch.cat([fake, agg_mask, self.sample_noise(B, L)], dim=1)
...
# ------------------- Imputation (Inference) -------------------
    def impute_csv(self, input_csv: str, output_csv: str, ckpt_path: str):
        print(f"=== Imputing missing values in {input_csv} using checkpoint {ckpt_path} ===")
        device = self.device
        ...
        with torch.no_grad():
            for start in tqdm(range(0, T, step)):
                ...
                # 替换缺失位置
                filled_normed[start:end][msk[:avail] == 0] = pred[:avail][msk[:avail] == 0]
...
class Config:
    def __init__(self):
        ...
        self.lr_g = 1e-4
        self.lr_d = 4e-4
        self.epochs = 250              # set higher for real training
...
def main():
    ...
    ckpt_path = "checkpoints/ckpt_ep249.pt"  # 选择你训练好的 epoch 模型
