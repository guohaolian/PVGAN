...
def _compute_day_to_window_index(unique_days: list, window_days: int, step_days: int) -> dict:
    """\
    Map each day to a window_labels index t.

    IMPORTANT: This mapping must be consistent with how we assign window labels to days.

    We compute windows as:
        for i in range(0, n_days-window_days+1, step_days):
            window_labels.append(label_for_window_start=i)

    In assign_window_labels_to_df() below, we assign each window label to *all days covered by that window*
    (with overlaps resolved by last-write-wins, i.e., a day uses the most recent window that covers it).

    Here we mirror that policy and return:
        dict[date] -> window_index (int)

    If no windows can be formed, all days map to -1.
    """
    ...
    if n_windows <= 0:
        return {day: -1 for day in unique_days}

    day_to_win: dict = {}

    # last-write-wins: later windows overwrite earlier mapping for overlapping days
    w_idx = 0
    for start in range(0, n_days - window_days + 1, step_days):
        end = start + window_days  # exclusive
        for di in range(start, end):
            day_to_win[unique_days[di]] = w_idx
        w_idx += 1

    # For trailing days not covered by any full window (possible when step_days>1),
    # map them to the last window so they still have a valid index.
    last_idx = n_windows - 1
    for day in unique_days:
        if day not in day_to_win:
            day_to_win[day] = last_idx
...
def build_argparser():
    ...
    # NOTE: store_true + default=True makes it impossible to disable from CLI.
    # Provide both --save-markov / --no-save-markov for clarity.
    p.add_argument(
        "--save-markov",
        dest="save_markov",
        action="store_true",
    ...
    p.add_argument(
        ...
        dest="save_markov",
        action="store_false",
        help="Do not save Markov transition sequence files.",
    )
    p.add_argument(
        "--markov-save-format",
        type=str,
...
def parse_args(argv: list[str] | None = None):
    ...
    # Current implementation assumes 3 clusters -> S/C/R mapping.
    if int(getattr(args, "kmeans_clusters", 3)) != 3:
        raise ValueError("--kmeans-clusters must be 3 for current S/C/R mapping. Please set it to 3.")
    return args
...
def assign_window_labels_to_df(
    df: pd.DataFrame,
    window_labels: list[str],
    *,
    window_days: int,
    ...
    """Assign each window label to all days covered by that window.

    Policy:
      - Window t corresponds to start_day_idx = t*step_days (as in compute_window_labels()).
      - That label is applied to every day in [start_day_idx, start_day_idx + window_days).
      - Overlaps are resolved by last-write-wins, so a day uses the most recent window that covers it.

    This greatly reduces None labels and keeps dataset buckets non-empty.
    """
    unique_days = sorted(df['date'].unique())
    n_days = len(unique_days)
    ...
    if n_days == 0 or window_days <= 0 or step_days <= 0:
        return df

    # number of windows implied by days; window_labels is computed the same way
    n_windows = max(0, (n_days - window_days) // step_days + 1)
    if n_windows <= 0 or len(window_labels) == 0:
        return df

    if len(window_labels) != n_windows:
        # Guardrail: avoid silent misalignment.
        raise ValueError(
            f"window_labels length mismatch: got {len(window_labels)} but expected {n_windows} "
            f"(n_days={n_days}, window_days={window_days}, step_days={step_days})."
        )
    ...
    for start in range(0, n_days - window_days + 1, step_days):
        ...
        lab = window_labels[w_idx]
        for di in range(start, end):
            day_to_label[unique_days[di]] = lab
        w_idx += 1

    # trailing days: if any day still has no label, use last window's label
    last_lab = window_labels[-1]
    for day in unique_days:
        if day not in day_to_label:
            day_to_label[day] = last_lab

    # assign back to rows
    df['window_label'] = df['date'].map(day_to_label)
    return df
...
def main(argv: list[str] | None = None):
    ...
    df = assign_window_labels_to_df(df, window_labels, window_days=int(args.window_days), step_days=int(args.step_days))
