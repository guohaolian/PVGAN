...
for epoch in range(epochs):
    for _ in range(5):  # Train Discriminator
        ...
        real_labels = torch.ones((batch_size, 1), device=device)  # 真样本标签
        fake_labels = torch.zeros((batch_size, 1), device=device)  # 假样本标签
    ...
    noise = torch.randn(batch_size, latent_dim).to(device)
    fake_data = generator(noise)
    g_loss = criterion(discriminator(fake_data), real_labels)  # 希望生成器生成的样本被判别器判为真
...
filled_data.to_csv("GAN/filled_data_01.csv", index=False)
print("缺失值已补全，并保存为 'GAN/filled_data.csv'.")
...
plt.legend()
plt.grid(True)
plt.savefig("GAN/loss_visualization_01.png")  # 保存图像
plt.show()

print("训练损失曲线已保存为 'GAN/loss_visualization.png'")
